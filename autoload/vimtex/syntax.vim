...
function! vimtex#syntax#init() abort " {{{1
  ...
  let l:tex_conceal = get(g:, 'tex_conceal', 'abdmgsS')
  ...
  let l:extfname = expand(':e')
  let b:tex_stylish = exists('g:tex_stylish')
        \ ? g:tex_stylish
        \ : get(b:, 'tex_stylish',
        \   index(['sty', 'cls', 'clo', 'dtx', 'ltx'], l:extfname) >= 0)

  let l:tex_comment_nospell = get(g:, 'tex_comment_nospell')
  let l:tex_matchcheck = get(g:, 'tex_matchcheck', '[({[]')
  let l:tex_excludematcher = get(g:, 'tex_excludematcher')
  ...
  if has('conceal') && &enc ==# 'utf-8' && l:tex_conceal =~# 'd'
  ...
  if !has('conceal') || &enc !=# 'utf-8' || l:tex_conceal !~# 'm'
...
" }}}1
function! s:SuperSub(group, leader, pat, cchar) " {{{1
  let l:tex_superscripts = get(g:, 'tex_superscripts', '[0-9a-zA-W.,:;+-<>/()=]')
  let l:tex_subscripts = get(g:, 'tex_subscripts', '[0-9aehijklmnoprstuvx,+-/().]')

  if a:pat =~# '^\\'
        \ || (a:leader ==# '\^' && a:pat =~# l:tex_superscripts)
        \ || (a:leader ==# '_' && a:pat =~# l:tex_subscripts)
...
" }}}1
function! s:Greek(group, pat, cchar) " {{{1
